...
root@ubuntu:~# # 1. CORREGIR NGINX (el problema principal)
cat > /etc/nginx/sites-enabled/bigartist << 'EOF'
server {
    listen 443 ssl http2;
    server_name app.bigartist.es;
    ...
    # API proxy - CORREGIDO con /api/ al final
    location /api/ {
        ...
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
curl https://app.bigartist.es/api/healthest_uri;
nginx: the configuration file /etc/nginx/nginx.conf syntax is ok
nginx: configuration file /etc/nginx/nginx.conf test is successful
{"status":"ok","timestamp":"2026-02-28T21:49:42.612Z"}root@ubuntu:~# 
